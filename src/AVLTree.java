...
//            System.out.println("z.getRightChild() = " + z.getRightChild());
    ...
    public AVLNode<T> getChildWithMaxHeight(AVLNode<T> x) {
        if (x == null) {throw new IllegalArgumentException("passed null instead of a valid node object");}
        
        AVLNode<T> leftChild = (AVLNode<T>)x.getLeftChild();  
        AVLNode<T> rightChild = (AVLNode<T>)x.getRightChild();
        AVLNode<T> answer = leftChild;        
        
        if (answer == null) {return rightChild;}                                        //if leftchild == null, return rightChild 
        
        if (rightChild != null) {                                                       //if leftChild != null and rightChild != null
            if (rightChild.getHeight() > leftChild.getHeight()) answer = rightChild;    //if height[r] > height[l] then answer = rightChild
        }
        return answer;
    }
    ...
    private void treeRotation(AVLNode<T> x) {
        if (x.getParent() == null) {throw new IllegalArgumentException("Invalid node, node x doesn't have any parent, cannot perform rotation");}
